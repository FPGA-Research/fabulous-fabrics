...
A fabric is any `fabrics/<name>/` holding a `fabric.yaml`, which names the tile
library the fabric is built from. The fabric's project skeleton sits beside it in
`common/` and one directory per HDL (`verilog/`, `vhdl/`). Adding such a directory
registers it; the registry scans on first access and caches the result for the life
of the process.
"""
...
import yaml
from fabulous_tiles import Language, Registry, TileLibrary, tile_libraries
...
class FabricSource:
    ...
    def from_dir(
        ...
        ValueError
            If `fabric.yaml` names a tile library missing from `libraries`.
        ...
        metadata = FabricMetadata.model_validate(
...
fabrics: Registry[FabricSource] = Registry(
    "fabric", lambda: load_fabrics(FABRICS_ROOT, tile_libraries)
)
